...
def get_projection_matrix(camera_matrix, rotation_matrix, translation_vector):
    ...
    return np.dot(camera_matrix, np.hstack((rotation_matrix, translation_vector[:, np.newaxis])))
...
if __name__ == "__main__":
    ...
    # Create 3D scatter plot for the 3D points
    fig = go.Figure(
    ...
    for i, (pos, ori) in enumerate(zip(cam_pos, cam_ori)):
        ...
        fig = go.Figure(data=[go.Heatmap(z=img, colorscale="gray", zmin=0, zmax=255)])
        fig.update_layout(title=f"Image {i+1}")
        fig.show()
